...
切换中文或 English。悬停/键盘聚焦划线文字查看详情，点击或 Enter 固定，Escape 关闭；触屏直接点按。一个片段有多个观点时，在详情顶部选择。默认直接显示结论和关键理由。补充解释、证据图、计算与发现按需展开，参考文献位于其中。详情保持打开，可把鼠标移进去滚动或访问来源。蓝色、虚线、点线标识不同证据状态，具体判断见文字。统计的是标注覆盖，不是观点正确率。
...
另附两篇人为构造的错误稿件，用于展示对误引、错误计算、模型依赖和来源指令的审读：`negative-case-blue-sticker-2026` 与 `microscopic-classical-description-2026`。它们是演示材料。后者引用的 Planck、Einstein、Millikan、Davisson–Germer 原刊是真实历史论文。

本框架和 skill 可整体搬运到其他 agent 工作目录；入口为 [SKILL.md](SKILL.md)。它们不依赖 Codex 专用工具。文献原文与各来源保留自己的版权归属；具体全文使用依据见数据中的 `paper.rights`。

## 许可

代码、skill、项目文档及本项目原创的示例与注释采用 [MIT License](LICENSE)。第三方论文原文、扫描件和原有译文按其各自权利状态使用，MIT 授权范围为本项目原创内容。随附历史原刊发表于 1901—1927 年，来源与作者见各论文 JSON；1905 年英译来自 Fourmilab 公版文本。依赖 AJV 与 KaTeX 各自采用 MIT 许可，其发行包保留相应许可文件。
