...
let paper, paperList = [], currentClaim = null, currentSegment = null, pinned = false, detailRequestId = 0, paperLoadId = 0, loadingPaper = false, hoverTimer;
const detailCache = new Map();
const t = key => ui[lang][key] || key;
Object.assign(ui.zh, { details: 'AI 解读', reasoning: '推导步骤', story: '从依据到结论', why: '判断理由', supporting: '查阅证据、计算与补充发现', tertiary: '参考文献与来源详情', provided_excerpt: '已读用户提供摘录', provided_full_text: '已读用户提供全文' });
Object.assign(ui.en, { details: 'AI READING', reasoning: 'Derivation steps', story: 'How the reasoning unfolds', why: 'Why this conclusion', supporting: 'Evidence, calculations & additional findings', tertiary: 'References & source details', provided_excerpt: 'User-provided excerpt inspected', provided_full_text: 'User-provided full text inspected' });
...
function renderPaper() {
  ...
  for (const status of ['supported', 'conditional', 'proposal']) legend.append(el('span', status, t(status)));
  ...
  paper.paragraphs.forEach((paragraph, index) => {
    ...
    const block = el('div', `paper-paragraph ${paragraph.type === 'equation' ? 'equation-paragraph' : ''} ${paragraph.type === 'footnote' ? 'footnote' : ''}`);
    block.id = paragraph.id;
    block.append(el('span', 'paragraph-number', String(index + 1).padStart(2, '0')));
    for (const [i, segment] of paragraph.segments.entries()) {
      ...
      mark.id = segment.id;
      if (segment.latex && window.katex) {
        try { window.katex.render(segment.latex, mark, { displayMode: paragraph.type === 'equation', throwOnError: true, trust: false, maxExpand: 500, maxSize: 20, strict: 'ignore' }); }
      ...
      if (segment.claim_ids.length) {
        ...
        mark.addEventListener('focus', () => { if (!pinned) showClaim(segment.claim_ids[0], segment.id); });
...
function renderEmpty() {
  ...
  if (paper?.claims.length) {
    ...
    empty.append(button(t('example'), () => { showClaim(primary.id, primary.segment_ids[0], { pin: true }); document.getElementById(primary.segment_ids[0])?.scrollIntoView({ block: 'center', behavior: 'smooth' }); }, 'example-button'));
  }
...
async function showClaim(id, segmentId, options = {}) {
  ...
  if (!detailCache.has(key)) {
    panel.replaceChildren(el('p', 'panel-body loading', t('loading')));
...
function renderDetail(detail) {
  ...
  const pin = button(t(pinned ? 'pinned' : 'pin'), () => { pinned = !pinned; renderDetail(detail); }, pinned ? 'pinned' : '');
  ...
  body.append(el('h2', '', bi(claim.title)), el('p', 'claim-summary', bi(claim.summary)));
  if (claim.why) {
    const why = el('div', 'why-box'); why.append(el('span', 'why-label', t('why')), el('p', '', bi(claim.why))); body.append(why);
  }
  const story = el('section', 'story-section'); story.append(el('h3', '', t('story')));
  for (const paragraph of claim.story || claim.reasoning.map(text => ({ text }))) story.append(el('p', '', bi(paragraph.text)));
  body.append(story);
  ...
  if (claim.reasoning.length) {
    const section = el('section', 'detail-section'); section.append(el('h3', '', t('reasoning')));
    const list = el('ol', 'reasoning-list'); claim.reasoning.forEach(r => list.append(el('li', '', bi(r)))); section.append(list); supporting.append(section);
  }
  ...
  function node(eid, depth = 0, edgeLabel) {
    if (shown.has(eid)) {
      ...
      shared.addEventListener('click', event => { event.preventDefault(); document.getElementById(`node-${eid}`)?.scrollIntoView({ block: 'nearest', behavior: 'smooth' }); });
  ...
  for (const source of sources) {
    ...
    if (source.local_path) item.append(el('p', '', ''), link(t('localSource'), `/${source.local_path}`, '', false));
    refs.append(item);
  }
