...
from open_access import find_open_pdf
...
def canonical_navigation_target(raw: str, *, deadline: float | None = None) -> str:
    target = validate_url(raw)
    parsed = urlparse(target)
    host = (parsed.hostname or "").lower().rstrip(".")
    if host_in_domains(host, ("wiley.com",)) and re.fullmatch(
        r"/doi/full-xml/10\.\d{4,9}/[^?#]+", parsed.path
    ):
        return parsed._replace(path=parsed.path.replace("/doi/full-xml/", "/doi/full/", 1)).geturl()
    ...
    resolve_timeout = DOI_RESOLVE_TIMEOUT_SECONDS
    if deadline is not None:
        resolve_timeout = min(resolve_timeout, deadline - time.monotonic())
        if resolve_timeout <= 0:
            raise LiteratureBrowserTimeoutError("DOI resolution exhausted the request budget")
    try:
        ...
        try:
            ...
            if type(exc).__name__ == "TimeoutError":
                raise LiteratureBrowserTimeoutError(
                    "publisher navigation timed out within the request budget"
                ) from exc
            if (
...
def classify_page(final_url: str, title: str, text: str) -> str:
    if title.strip().casefold() in {"captcha", "human verification"}:
        return "challenge"
    if urlparse(final_url).hostname == "validate.perfdrive.com":
        return "challenge"
...
def content_access_denied(text: str) -> bool:
    return bool(re.search(
        r"you do not (?:currently )?have access to this content|"
        r"you don't (?:currently )?have access to this content",
        text, re.IGNORECASE,
    ))


def full_text_visible(text: str) -> bool:
    if content_access_denied(text):
        return False
...
FULL_TEXT_DOM_SCRIPT = """() => {
  const denied = /you do not (?:currently )?have access to this content|you don't (?:currently )?have access to this content/i;
  if (denied.test(document.body?.innerText || '')) return false;
  const selectors = '.widget-ArticleFulltext, .article-body, .article__body, '
    + '.article-section__content, .Body, #body, .c-article-body, '
    + '[data-aa-name="articleBody"]';
  return Array.from(document.querySelectorAll(selectors)).some((node) => {
    const copy = node.cloneNode(true);
    copy.querySelectorAll('nav, aside, header, footer, .abstract, #abstract, '
      + '.references, #references, .ref-list, .bibliography, '
      + '[role="doc-bibliography"], [role="doc-abstract"]').forEach(el => el.remove());
    const paragraphs = Array.from(copy.querySelectorAll('p'))
      .map(el => (el.textContent || '').trim()).filter(text => text.length >= 100);
    return paragraphs.length >= 3 && paragraphs.join(' ').length >= 8000;
  });
}"""
...
def article_fragment(nodes: list[Any], final_url: str) -> tuple[str, str]:
    """Return inert article markup and block-separated text from the same nodes."""
    from bs4 import BeautifulSoup, Comment

    selected = {id(node) for node in nodes}
    roots = [node for node in nodes if not any(id(p) in selected for p in node.parents)]
    fragment = BeautifulSoup("\n".join(str(node) for node in roots), "html.parser")
    for node in fragment.select("script, style, noscript, iframe, object, embed, form, input, button, link, meta, base"):
        node.decompose()
    for comment in fragment.find_all(string=lambda value: isinstance(value, Comment)):
        comment.extract()
    attributes = {"id", "class", "title", "alt", "href", "src", "colspan", "rowspan", "scope",
                  "headers", "role", "aria-label", "display", "encoding", "xmlns", "mathvariant",
                  "stretchy", "fence", "separator", "accent", "accentunder", "linethickness"}
    for node in fragment.find_all(True):
        lazy_src = node.get("data-src") or node.get("data-original")
        if node.name == "img" and lazy_src:
            node["src"] = lazy_src
    ...
    for node in fragment.find_all(["p", "div", "section", "article", "header", "footer", "aside",
                                   "h1", "h2", "h3", "h4", "h5", "h6", "li", "figure", "figcaption", "table", "tr"]):
        node.insert_before("\n\n")
        node.insert_after("\n\n")
    for node in fragment.find_all("br"):
        node.replace_with("\n")
    for node in fragment.find_all(["td", "th"]):
        node.insert_after("\t")
    text = fragment.get_text()
    text = re.sub(r"[^\S\n]+", " ", text)
    text = re.sub(r" *\n *", "\n", text)
    text = re.sub(r"\n{3,}", "\n\n", text).strip()
    return article_html, text
...
def extract_structured_html_article(
    ...
    for node in soup.select(
        "script, style, noscript, nav, form, button"
    ):
        node.decompose()
    selectors = (
        "article",
    ...
    for selector in selectors:
        ...
        if not nodes:
            continue
        markup_fragment, text = article_fragment(nodes, final_url)
        candidate_html[selector] = markup_fragment
        if text:
    ...
    raw_chars = len(text)
    text, text_truncated = trim_text(text, max_chars)
    article_html = candidate_html.get(selector, "") if usable else ""
    return {
        "attempted": True,
        "success": usable,
        "selector": selector,
        ...
        "article_html_source_url": final_url if article_html else "",
        "article_html_selector": selector if article_html else "",
        "article_html_truncated": False,
...
def classify_access(
    ...
    *,
    ...
    if content_access_denied(text) or signal_set & {
        "sign in via your institution",
        "get access",
        "purchase access",
    }:
...
def select_pdf_candidates(links: list[dict[str, str]]) -> list[str]:
    ...
    for item in links:
        ...
        citation_pdf = item.get("source") == "meta" and label == "citation_pdf_url"
        if not citation_pdf and not re.search(r"\bpdf\b|\.pdf(?:[?#]|$)", combined):
            continue
...
def publisher_figure_source(source: str, article_url: str, image_hint: str) -> str:
    article = urlparse(article_url)
    hint = urlparse(image_hint)
    original = urlparse(urljoin(article_url, source))
    if (host_in_domains(article.hostname or "", ("mdpi.com",))
            and hint.scheme == "https" and hint.hostname == "pub.mdpi-res.com"
            and original.hostname == article.hostname
            and "/article_deploy/html/images/" in hint.path
            and original.path.rsplit("/", 1)[0] == hint.path.rsplit("/", 1)[0]):
        return validate_url(hint._replace(path=original.path, query=original.query, fragment="").geturl())
    return validate_url(urljoin(article_url, source)) if source else ""
...
def extract_figure_manifest(
    ...
    *,
    ...
    image_hint = page.locator('meta[property="og:image"]').first.get_attribute("content") if page.locator('meta[property="og:image"]').count() else ""
    for figure in value["items"]:
        try:
            figure["src"] = publisher_figure_source(str(figure.get("src") or ""), page.url, image_hint or "")
        except LiteratureBrowserError:
            pass
    return value
...
def extract_figure_manifest_from_markup(
    ...
    *,
    ...
    def add_candidate(container: Any, image: Any, method: str) -> None:
        ...
        if source:
            try:
                hint_node = soup.select_one('meta[property="og:image"]')
                source = publisher_figure_source(html.unescape(source), final_url, str(hint_node.get("content") or "") if hint_node else "")
            except LiteratureBrowserError:
                return
    ...
    *,
    ...
    for figure in figures:
        ...
        try:
            ...
            pixels = locator.evaluate("""async image => {
              if (!(image instanceof HTMLImageElement)) {
                throw new Error('No decoded HTML image is available; page screenshots are not delivered as figures');
              }
              let timer;
              try {
                await Promise.race([
                  image.decode(),
                  new Promise((_, reject) => { timer = setTimeout(() => reject(new Error('Image decode timed out')), 10000); })
                ]);
              } finally { clearTimeout(timer); }
              const width = image.naturalWidth, height = image.naturalHeight;
              if (!image.complete || !width || !height) throw new Error('Image did not load completely');
              if (width * height > 16000000) throw new Error('Decoded figure exceeds the pixel limit');
              const canvas = document.createElement('canvas');
              canvas.width = width; canvas.height = height;
              canvas.getContext('2d').drawImage(image, 0, 0, width, height);
              return { width, height, data: canvas.toDataURL('image/png').split(',')[1] };
            }""")
            raw = base64.b64decode(pixels["data"], validate=True)
            if not raw or len(raw) > MAX_FIGURE_IMAGE_BYTES:
                raise LiteratureBrowserError("decoded figure exceeds the image size limit")
            figure["image_base64"] = base64.b64encode(raw).decode("ascii")
            figure["mime_type"] = "image/png"
            figure["image_bytes"] = len(raw)
            figure["image_width"] = pixels["width"]
            figure["image_height"] = pixels["height"]
            figure["image_integrity"] = "complete_intrinsic_pixels"
            figure["image_extraction_method"] = "browser_decoded_image_png"
            figure["image_access_state"] = "available_decoded"
        except Exception as exc:
            decode_error = re.sub(r"\s+", " ", str(exc)).strip()[:180]
            figure["image_integrity"] = "unavailable"
            figure["image_error"] = "; ".join(
                part for part in (fetch_error, decode_error) if part
            )[:400]
...
def publisher_pdf_http_method(target: str) -> str:
    parsed = urlparse(target)
    if (host_in_domains(parsed.hostname or "", ("annualreviews.org",))
            and parsed.path.startswith("/deliver/fulltext/")
...
def fetch_pdf_in_browser(
    ...
    *,
    ...
    result = page.evaluate(
        """async ({url, maxBytes, timeoutMs, method}) => {
        ...
        }""",
        {
            ...
            "method": publisher_pdf_http_method(url),
        },
    )
    if not isinstance(result, dict):
        raise LiteratureBrowserError(
    ...
    *,
    ...
    try:
        ...
        try:
            ...
            pass
        request = page.request.post if publisher_pdf_http_method(target) == "POST" else page.request.get
        response = request(
            target,
            headers=headers,
            timeout=timeout_ms,
...
def mdpi_article_pdf_url(target: str) -> bool:
    parsed = urlparse(target)
    return (parsed.scheme == "https"
            and host_in_domains(parsed.hostname or "", ("mdpi.com",))
            and bool(re.fullmatch(r"/\d{4}-\d{3}[\dX]/\d+/\d+/\d+/pdf/?", parsed.path)))
...
def fetch_pdf_via_browser_download(
    ...
    *,
    ...
    url = validate_url(target)
    host = urlparse(url).hostname or ""
    if not browser_pdf_fetch_host_allowed(host) or not (is_supplementary_pdf_url(url) or mdpi_article_pdf_url(url)):
        raise LiteratureBrowserError(
            "browser downloads are restricted to governed PDF routes"
        )
    ...
    try:
        ...
        source = validate_browser_pdf_result_url(url, download.url)
        validate_pdf_payload(raw, "", 200)
        return raw, "application/pdf", source, 200
    except LiteratureBrowserError:
        raise
    except Exception as exc:
        raise LiteratureBrowserError(
...
def is_supplementary_pdf_url(target: str) -> bool:
    ...
    if host_in_domains(host, ("mdpi.com",)) and re.search(r"/s\d+/?$", path):
        return True
...
def validate_pdf_payload(raw_pdf: bytes, content_type: str, status: int) -> None:
    ...
    if "application/pdf" not in content_type and not raw_pdf.startswith(b"%PDF"):
        raise LiteratureBrowserError(
            "publisher link did not return a PDF"
        )


def download_article_pdf(
    page: Any, links: list[dict[str, str]], *, deadline: float,
) -> dict[str, Any]:
    candidates = select_pdf_candidates(links)[:MAX_PDF_CANDIDATES]
    seen = set(candidates)
    errors: list[str] = []
    for target in candidates:
        for strategy in (("download", "browser", "context") if mdpi_article_pdf_url(target) else ("browser", "context")):
            remaining_ms = int((deadline - time.monotonic()) * 1000)
            if remaining_ms < 1000:
                return {"success": False, "reason": "time_budget_exhausted", "errors": errors}
            try:
                if strategy == "download":
                    raw, kind, source, status = fetch_pdf_via_browser_download(
                        page, target, timeout_ms=min(20000, remaining_ms),
                    )
                elif strategy == "browser":
                    raw, kind, source, status = fetch_pdf_in_browser(
                        page, target, timeout_ms=min(15000, remaining_ms),
                    )
                else:
                    raw, kind, source, status = fetch_pdf_with_context_request(
                        page, target, timeout_ms=min(15000, remaining_ms),
                        browser_error=errors[-1] if errors else "",
                    )
                if not raw.startswith(b"%PDF-") and status < 400:
                    for linked in extract_pdf_urls_from_viewer(raw, source):
                        if linked not in seen and len(candidates) < 6:
                            candidates.append(linked)
                            seen.add(linked)
                validate_pdf_payload(raw, kind, status)
                if not raw.startswith(b"%PDF-"):
                    raise LiteratureBrowserError("response is not a PDF file")
                from pypdf import PdfReader
                pages = len(PdfReader(io.BytesIO(raw)).pages)
                if pages == 0:
                    raise LiteratureBrowserError("PDF contains no pages")
                parsed_source = urlparse(source)
                public_query = urlencode([
                    (key, value) for key, value in parse_qsl(parsed_source.query)
                    if key in {"id", "type"}
                ])
                source = parsed_source._replace(
                    query=public_query, fragment="", netloc=parsed_source.hostname or "",
                ).geturl()
                return {"success": True, "source_url": source, "bytes": len(raw),
                        "pages": pages, "data_base64": base64.b64encode(raw).decode("ascii")}
            except Exception as exc:
                detail = re.sub(r"https?://\S+", "[publisher URL]", str(exc))
                errors.append(f"{strategy}: {type(exc).__name__}: {detail}"[:300])
    return {"success": False, "reason": "fetch_failed" if candidates else "no_article_pdf_link",
            "errors": errors}


def open_version_doi(original_url: str, result: dict[str, Any]) -> str:
    def normalize(value: Any) -> str:
        if not isinstance(value, str):
            return ""
        value = unquote(value).strip()
        value = re.sub(r"^(?:https?://(?:dx\.)?doi\.org/|doi:\s*|info:doi/)", "", value, flags=re.I)
        return value if re.fullmatch(r"10\.\d{4,9}/[^\s<>\"?#]+", value) else ""

    def from_url(value: str) -> str:
        try:
            parsed = urlparse(value)
        except ValueError:
            return ""
        ...
        found = re.search(r"/(10\.\d{4,9}/[^?#]+)$", path)
        if found:
            return normalize(re.sub(r"\.pdf$", "", found.group(1), flags=re.I))
        for key, item in parse_qsl(parsed.query):
            if key.lower() in {"doi", "id"}:
                doi = normalize(item)
                if doi:
                    return doi
        return ""

    doi = from_url(original_url)
    if doi:
        return doi
    metadata = result.get("metadata")
    values: dict[str, str] = {}
    if isinstance(metadata, dict):
        for key in ("citation_doi", "prism.doi", "dc.identifier", "dc.identifier.doi", "doi"):
            raw = metadata.get(key, [])
            for item in raw if isinstance(raw, list) else [raw]:
                doi = normalize(item)
                if doi:
                    values[doi.casefold()] = doi
    if len(values) == 1:
        return next(iter(values.values()))
    if values:
        return ""
    return from_url(str(result.get("final_url") or ""))


def article_open_fallback_allowed(url: str) -> bool:
    parsed = urlparse(url)
    path = unquote(parsed.path).lower()
    if is_supplementary_pdf_url(url):
        return False
    return not (
        any(marker in path for marker in SUPPLEMENTARY_PDF_PATH_MARKERS)
        or SUPPLEMENTARY_PDF_FILENAME_RE.search(path)
        or re.search(r"(?:-mmc\d+|/suppl/|/suppdata/|/action/downloadsupplement)", path)
        or (parsed.hostname or "").lower() == "acs.figshare.com"
    )


def approved_open_url(url: str, *, pdf: bool = False) -> bool:
    try:
        target = validate_url(url)
    except LiteratureBrowserError:
        return False
    return not pdf or browser_pdf_fetch_host_allowed(urlparse(target).hostname or "")


def safe_page_title(page: Any) -> str:
    try:
        return str(page.locator("head > title").text_content(timeout=3000) or "").strip()
    except Exception:
        return ""


def stop_page_loading(page: Any) -> None:
...
def page_full_text_dom_visible(page: Any) -> bool:
    try:
        return bool(page.locator("body").evaluate(FULL_TEXT_DOM_SCRIPT, timeout=5000))
    except Exception:
        return False


def recover_wiley_navigation(page: Any, target: str, *, deadline: float) -> tuple[Any, dict[str, Any]]:
    def remaining_ms() -> int:
        remaining = int((deadline - time.monotonic()) * 1000)
        if remaining < 1000:
            raise LiteratureBrowserTimeoutError("Wiley recovery reached the request time budget")
        return remaining

    remaining_ms()
    stop_page_loading(page)
    response = goto_with_dns_retry(page, target, deadline=deadline)
    timeout_ms = min(DOM_READY_TIMEOUT_MS, remaining_ms())
    try:
        page.wait_for_load_state("domcontentloaded", timeout=timeout_ms)
    except Exception:
        stop_page_loading(page)
    sample = page_access_sample(page, body_timeout_ms=min(5000, remaining_ms()))
    remaining_ms()
...
def extract_pdf_urls_from_viewer(
    ...
    for value in values:
        value = value.strip()
        if not re.search(r"/doi/(?:pdfdirect|pdf|am-pdf)/|\.pdf(?:[?#]|$)", value, flags=re.IGNORECASE):
            continue
        target = urljoin(base_url, value)
        source_doi = re.search(r"/doi/(?:e?pdf|pdfdirect|am-pdf)/(10\.\d{4,9}/.+)", unquote(urlparse(base_url).path), re.IGNORECASE)
        target_doi = re.search(r"/doi/(?:e?pdf|pdfdirect|am-pdf)/(10\.\d{4,9}/.+)", unquote(urlparse(target).path), re.IGNORECASE)
        if source_doi and target_doi and source_doi[1].casefold() != target_doi[1].casefold():
            continue
...
class BrowserManager:
    ...
    def _read_in_worker(
        ...
        *,
        ...
        options = {
            "wait_ms": wait_ms, "max_chars": max_chars,
            "include_figure_images": include_figure_images, "max_figures": max_figures,
            "figure_offset": figure_offset, "timeout_seconds": timeout_seconds,
            "landing_url_hint": landing_url_hint,
            "maintenance_session_refresh": maintenance_session_refresh,
            "download_pdf": download_pdf,
        }
        target = validate_url(url)
        if not download_pdf or maintenance_session_refresh or not article_open_fallback_allowed(target):
            return self._read_publisher_in_worker(target, **options)
        budget = max(5, min(int(timeout_seconds), READ_BUDGET_SECONDS))
        deadline = time.monotonic() + budget
        publisher_deadline = deadline - min(35, budget * 0.4)
        try:
            result = self._read_publisher_in_worker(target, deadline=publisher_deadline, **options)
        except (SameSessionDownloadError, SameSessionAssetError):
            raise
        except Exception as exc:
            message = re.sub(r"https?://\S+", "[publisher URL]", str(exc))
            result = {
                "success": False, "url": target, "final_url": target,
                "status": None, "page_state": "error", "access_state": "error",
                "text": "", "text_truncated": False,
                "pdf_download": {
                    "success": False, "reason": "publisher_request_failed",
                    "errors": [f"{type(exc).__name__}: {message}"[:400]],
                },
            }
        result["url"] = target
        if result.get("page_state") == "challenge":
            parsed = urlparse(str(result.get("final_url") or target))
            result["final_url"] = parsed._replace(query="", fragment="").geturl()
        publisher_pdf = result.get("pdf_download") or {
            "success": False, "reason": "publisher_pdf_unavailable",
        }
        if publisher_pdf.get("success"):
            result["open_access"] = {"attempted": False, "reason": "requested_pdf_available"}
            return result
        result["publisher_attempt"] = {
            "page_state": result.get("page_state"), "access_state": result.get("access_state"),
            "status": result.get("status"), "pdf_download": publisher_pdf,
        }
        doi = open_version_doi(target, result)
        if not doi:
            result["pdf_download"] = publisher_pdf
            result["open_access"] = {"attempted": False, "reason": "doi_unavailable"}
            return result
        pdf, lookup = find_open_pdf(
            doi, deadline=deadline, max_bytes=MAX_PDF_BYTES,
            allowed_url=approved_open_url,
            pdf_url_allowed=lambda candidate: approved_open_url(candidate, pdf=True),
        )
        result["open_access"] = lookup
        if not pdf.get("success"):
            result["pdf_download"] = {
                **publisher_pdf, "open_version_reason": pdf.get("reason", "no_verified_open_pdf"),
            }
            return result
        existing_text = str(result.get("text") or "")
        preserve_text = bool(existing_text.strip()) and not result.get("text_truncated") and result.get("access_state") in {
            "institutional_full_text", "publisher_full_text", "open_access_full_text", "full_text_visible",
        }
        text_fields = {
            key: result[key] for key in (
                "text", "text_truncated", "text_source", "access_state", "references",
                "html_text_chars", "full_text_dom_visible", "html_extraction", "figures", "figure_extraction",
            ) if key in result
        }
        text_fields["text_source_url"] = result.get("text_source_url") or result.get("final_url") or target
        result.update(
            success=True, final_url=pdf["source_url"], status=200, page_state="content",
            access_state="open_access_pdf", text="", text_truncated=False,
            text_source="open_version_pdf", text_source_url=pdf["source_url"], title=pdf.get("title") or doi,
            metadata={"citation_doi": [doi]}, figures=[], references=[],
            pdf_links=[{"url": pdf["source_url"], "label": "Open version PDF"}],
            pdf_download=pdf, html_text_chars=0, full_text_dom_visible=False,
            access_signals=["verified_open_version"], institution_markers=[],
            figure_extraction={"total": 0, "offset": 0, "returned": 0, "has_more": False},
            html_extraction={"attempted": False},
        )
        try:
            text, truncated, pages = extract_pdf_text(
                base64.b64decode(pdf["data_base64"], validate=True), max_chars,
            )
            result.update(
                text=text, text_truncated=truncated,
                access_state="open_access_full_text" if text.strip() else "open_access_pdf",
                references=extract_numbered_references(text),
                pdf_extraction={"attempted": True, "success": True,
                                "pages": pages, "bytes": pdf["bytes"],
                                "source_url": pdf["source_url"], "text_chars": len(text)},
            )
            if not text.strip() and preserve_text:
            ...
            result["pdf_extraction"] = {
                ...
                "message": f"Open PDF text extraction failed: {type(exc).__name__}",
            }
        return result

    def _read_publisher_in_worker(
        self,
        url: str,
        *,
        wait_ms: int,
        ...
        if deadline is None:
            deadline = time.monotonic() + max(5, min(int(timeout_seconds), READ_BUDGET_SECONDS))
        target = canonical_navigation_target(url, deadline=deadline)
        ...
        with self._lock:
            ...
            try:
                if host_in_domains(
                    urlparse(target).hostname or "",
                    ("sciencedirect.com", SCIENCEDIRECT_CHINA_STATIC_HOST),
                ):
                    page.route(
                        f"https://{SCIENCEDIRECT_CHINA_STATIC_HOST}/**",
                        proxy_sciencedirect_static_asset,
                    )
                if mdpi_article_pdf_url(target):
                    remaining_ms = int((deadline - time.monotonic()) * 1000)
                    if remaining_ms < 1000:
                        raise LiteratureBrowserTimeoutError("PDF download reached the request budget")
                    raw, kind, source, status = fetch_pdf_via_browser_download(
                        page, target, timeout_ms=min(20000, remaining_ms),
                    )
                    text, truncated, pages = extract_pdf_text(raw, max_chars)
                    result = article_pdf_result(target, {
                        "text": text, "text_truncated": truncated, "pages": pages,
                        "source_url": urlparse(source)._replace(query="", fragment="").geturl(),
                        "status": status, "bytes": len(raw), "strategy": "browser_download",
                        **pdf_page_payload(raw, pages, figure_offset if include_figure_images else None),
                    })
                    if download_pdf:
                        result["pdf_download"] = {"success": True, "source_url": result["final_url"],
                            "bytes": len(raw), "pages": pages,
                ...
                profile_warm: dict[str, Any] = {"attempted": False}
                navigation_recovery: dict[str, Any] = {"attempted": False}
                ...
                wiley_article = (
                    host_in_domains(
                        urlparse(str(initial_sample.get("url") or target)).hostname or "",
                        ("wiley.com",),
                    )
                    and not target_is_supplementary_pdf
                    and not original_is_article_pdf_request
                )
                sample_text = str(initial_sample.get("text") or "")
                if wiley_article and (
                    not sample_text.strip()
                    or "verification successful" in sample_text.casefold()
                ):
                    navigation_recovery = {
                        "attempted": True,
                        "same_profile": True,
                        "reason": (
                            "verified_challenge_redirect"
                            if sample_text.strip() else "incomplete_document"
                        ),
                    }
                    response, initial_sample = recover_wiley_navigation(
                        page, target, deadline=deadline,
                    )
                    navigation_recovery["document_loaded"] = bool(
                        str(initial_sample.get("text") or "").strip()
                    )
                ...
                try:
                    ...
                    html_text = ""
                if wiley_article and not html_text.strip():
                    stop_page_loading(page)
                    raise LiteratureBrowserError(
                        "Wiley document remained incomplete after same-profile navigation recovery"
                    )
                ...
                pdf_links = page.evaluate(
                    """() => {
                      ...
                      for (const node of document.querySelectorAll(
                        'a[href], iframe[src], embed[src], object[data], form[action*="/deliver/fulltext/"], '
                ...
                if (
                    ...
                    try:
                        ...
                        if html_extraction.get("success"):
                            ...
                            truncated = bool(html_extraction.get("text_truncated"))
                ...
                if (
                    ...
                    for candidate in candidates:
                        ...
                        try:
                            ...
                            pdf_extraction.pop("message", None)
                            attempt["accepted"] = True
                            break
                        except Exception as exc:
                            attempt["error_type"] = type(exc).__name__
                            attempt["message"] = str(exc)[:400]
                            pdf_extraction["message"] = (
                ...
                result = {
                    ...
                    **{key: html_extraction.pop(key) for key in list(html_extraction)
                       if key.startswith("article_html")},
                ...
                if download_pdf:
                    result["pdf_download"] = download_article_pdf(
                        page, pdf_links, deadline=deadline,
                    )
                    if (result["pdf_download"].get("success")
                            and result["access_state"] == "metadata_or_abstract"):
                        try:
                            downloaded = result["pdf_download"]
                            pdf_text, pdf_truncated, pdf_pages = extract_pdf_text(
                                base64.b64decode(downloaded["data_base64"], validate=True), max_chars,
                            )
                            if pdf_text.strip():
                                result.update(
                                    text=pdf_text, text_truncated=pdf_truncated, text_source="pdf",
                                    access_state=classify_access("content", pdf_text, signals, full_text_dom=True),
                                    pdf_extraction={"attempted": True, "success": True,
                                                    "pages": pdf_pages, "bytes": downloaded["bytes"],
                                                    "source_url": downloaded["source_url"],
                                                    "text_chars": len(pdf_text)},
                                    references=extract_numbered_references(pdf_text),
                                )
                        except Exception as exc:
                            result["pdf_extraction"] = {
                                "attempted": True, "success": False,
                                "message": f"Downloaded PDF text extraction failed: {type(exc).__name__}",
                            }
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            download_pdf = payload.get("download_pdf", False)
            if not isinstance(download_pdf, bool):
                raise LiteratureBrowserError("download_pdf must be a boolean")
