...
cleanup() {
    ...
    [ -z "$webvnc_pid" ] || kill "$webvnc_pid" 2>/dev/null || true
...
if [ "${LITERATURE_BROWSER_MANUAL_VIEW:-false}" = "true" ]; then
    x11vnc -display "$display" -rfbport 5900 -listen 127.0.0.1 \
        -forever -shared -nopw -noxdamage -quiet >/tmp/manual-view.log 2>&1 &
    vnc_pid=$!
    websockify --web /usr/share/novnc 6080 127.0.0.1:5900 >/tmp/manual-web.log 2>&1 &
    webvnc_pid=$!
fi
